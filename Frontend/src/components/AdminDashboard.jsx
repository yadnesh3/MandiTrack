...
export default function AdminDashboard({ user }) {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      {error && (
        <div className="p-4 rounded-xl bg-[#FAEEEE] border border-[#E8C9C9] text-[#8F3F3F] text-xs font-bold">
          {error}
        </div>
      )}

      {successMsg && (
        <div className="p-4 rounded-xl bg-[#EAF2E9] border border-[#C9DDCC] text-[#214D31] text-xs font-bold flex items-center justify-between">
          <span>✓ {successMsg}</span>
          <button onClick={() => setSuccessMsg("")} className="text-[#285C3A]">
            ✕
          </button>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl p-6 sm:p-8 border border-[#DCE3DB] shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#E7EBE5] pb-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#F5EFDE] text-[#B58A35] text-xs font-bold border border-[#E8DDBF] mb-1">
              <Lock size={13} />
              Officer ID Security & Control
            </div>
            <h2 className="text-xl font-bold text-[#19343A] tracking-tight">
              Create Authorized Mandi Officer
            </h2>
            <p className="text-xs text-[#687779] font-medium mt-0.5">
          ...
          <span className="px-3 py-1 rounded-xl bg-[#F8F7F2] text-[#19343A] text-xs font-bold">
            Server-Enforced Access
          </span>
        </div>

        {officerFormError && (
          <div className="p-3.5 rounded-xl bg-[#FAEEEE] border border-[#E8C9C9] text-[#8F3F3F] text-xs font-bold flex items-center gap-2">
            <AlertCircle size={16} />
        ...
        <form onSubmit={handleCreateOfficerSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 items-end">
          <div>
            <label className="block text-[11px] font-bold uppercase text-[#687779] mb-1">
              Officer ID *
            </label>
            <input
              type="text"
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-[#DCE3DB] bg-white text-xs font-semibold font-mono focus:outline-none focus:ring-2 focus:ring-[#EAF2E9] focus:border-[#285C3A] transition"
              required
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl border border-[#DCE3DB] bg-white text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-[#EAF2E9] focus:border-[#285C3A] transition"
              required
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2.5 rounded-xl border border-[#DCE3DB] bg-white text-xs font-semibold text-[#19343A] focus:outline-none focus:ring-2 focus:ring-[#EAF2E9] focus:border-[#285C3A] transition"
              required
            >
              {MANDI_LIST.map((m) => (
          ...
          <div className="sm:col-span-2 lg:col-span-5 pt-2 flex justify-end">
            <button
              ...
              className="px-5 py-2.5 rounded-lg bg-[#285C3A] hover:bg-[#214D31] text-white font-semibold text-xs shadow-sm transition active:scale-[0.98] disabled:opacity-50 flex items-center gap-2"
            >
              <PlusCircle size={15} />
        ...
        <div className="pt-4 border-t border-[#E7EBE5] space-y-3">
          <h3 className="text-xs font-bold text-[#19343A] uppercase tracking-wider">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#E7EBE5] text-[11px] font-bold uppercase text-[#8A9695]">
              ...
              <tbody className="divide-y divide-[#E7EBE5] text-xs font-semibold text-[#19343A]">
                {officers.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-4 text-center text-[#8A9695] font-medium">
                      No officers created yet. Use the form above to issue the first Officer ID.
                    </td>
                  </tr>
                ) : (
                  officers.map((off) => (
                    <tr key={off._id || off.officerId} className="hover:bg-[#F8F7F2]">
                      <td className="py-3 px-3 font-mono font-bold text-[#B58A35]">
                        {off.officerId || `OFF-${off._id?.slice(-4).toUpperCase()}`}
                      </td>
                      <td className="py-3 px-3 font-semibold text-[#19343A]">
                        {off.name}
                      </td>
                      <td className="py-3 px-3 text-[#687779] font-mono">
                        {off.mobile}
                      </td>
                      <td className="py-3 px-3 font-bold text-[#19343A]">
                        {off.mandi || "Pune APMC"}
                      </td>
                      <td className="py-3 px-3 text-right">
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-[#EAF2E9] text-[#285C3A] border border-[#C9DDCC]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-white rounded-xl p-6 border border-[#DCE3DB] shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-[#E7EBE5] pb-3">
            <h3 className="text-sm font-bold text-[#19343A] uppercase tracking-wider">
              APMC Mandi Volume Oversight
            </h3>
            <span className="text-xs font-bold text-[#687779]">Real-time DB Counts</span>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[#E7EBE5] text-xs font-semibold text-[#19343A]">
                {(data?.mandiOverview && data.mandiOverview.length > 0
                  ...
                  <tr key={i} className="hover:bg-[#F8F7F2]">
                    <td className="py-3 px-3 font-semibold text-[#19343A]">{m._id || "APMC"}</td>
                    <td className="py-3 px-3 text-center font-mono font-bold text-[#19343A]">{m.totalLots}</td>
                    <td className="py-3 px-3 text-center font-mono font-bold text-[#B58A35]">{m.pendingLots}</td>
                    <td className="py-3 px-3 text-right font-mono font-bold text-[#285C3A]">{m.soldLots}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Lots by Stage */}
        <div className="lg:col-span-5 bg-white rounded-xl p-6 border border-[#DCE3DB] shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-[#E7EBE5] pb-3">
            <h3 className="text-sm font-bold text-[#19343A] uppercase tracking-wider">
              Lots Distribution by Stage
            </h3>
            <span className="text-xs font-bold text-[#687779]">Live Stages</span>
          </div>
          ...
          <div className="space-y-2.5">
            {STAGES.map((stg) => {
              ...
              return (
                <div key={stg} className="flex items-center justify-between p-2.5 rounded-xl bg-[#F8F7F2] border border-[#E1E4DE] text-xs font-semibold">
                  <span className="text-[#19343A]">{stg}</span>
                  <span className="px-2.5 py-0.5 rounded-full bg-[#EAF2E9] font-mono text-[#285C3A] text-[11px]">
                    {count} lots
                  </span>
                </div>
              );
